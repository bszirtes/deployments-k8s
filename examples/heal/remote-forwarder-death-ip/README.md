...
Deploy NSC and NSE:
```bash
kubectl apply -k https://github.com/bszirtes/deployments-k8s/examples/heal/remote-forwarder-death-ip?ref=98d12834ac8b2af512570763c7d6eccb2151f778
```

Wait for applications ready:
```bash
kubectl wait --for=condition=ready --timeout=1m pod -l app=nettools -n ns-remote-forwarder-death-ip
```
```bash
...
```bash
kubectl exec pods/nettools -n ns-remote-forwarder-death-ip -- ping -c 4 172.16.1.100
```
